...
def parse_args():
    ...
    p.add_argument(
        ...
        help="1: per-sample mask; 0: normal SGD"
    )
    p.add_argument("--model_name", type=str, required=True)

    # correct: only one (mean-grad + evaluation)
    p.add_argument("--correct_jsonl", type=str, required=True)

    # wrong: split into two
    p.add_argument("--wrong1_jsonl", type=str, required=True)  # train
    p.add_argument("--wrong2_jsonl", type=str, required=True)  # eval
...
WRONG1_JSONL = args.wrong1_jsonl
WRONG2_JSONL = args.wrong2_jsonl
...
# ======================= train (wrong1 only) =======================
def train_wrong_per_sample_mask(
    model, tok, wrong_rows, lr, desc,
    named_params, grad_ref_gpu=None
):
    model.train()
    loss_sum = 0.0
    n = len(wrong_rows)
...
def main():
    ...
    wrong1_data = read_jsonl(WRONG1_JSONL)
    wrong2_data = read_jsonl(WRONG2_JSONL)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        train_loss = train_wrong_per_sample_mask(
            model, tok, wrong1_data, LR,
            desc=f"train wrong1 ep{ep}",
            named_params=named_params,
            grad_ref_gpu=grad_c_ref_gpu if USE_FREEZE else None
        )

        # wrong2 → correct
        w2_ok = infer_and_dump(
            model, tok, wrong2_data, cand_ids,
            os.path.join(JSONL_DIR, f"infer_wrong2_ep{ep}.jsonl"),
            f"infer wrong2 ep{ep}"
        )

        # correct → wrong (forgetting)
        c_ok = infer_and_dump(
